...
const PipelineListPanel = () => {
    ...
    const addStage = e => {
        e.preventDefault();
        setPipeline({ ...pipeline, stages: [...pipeline.stages, stageInitialState] });
        e.target.blur();
    };
        ...
        <div className="w-full lg:w-1/3 h-screen overflow-y-hidden bg-indigo-900 flex flex-col">
            ...
            { // TODO: extract to its own component?
                showAddModal ? <Modal close={() => setShowAddModal(false)}>
                    <div className="w-full h-full lg:w-2/5 lg:h-4/5 mx-auto shadow-sm bg-stone-800 md:rounded-md flex flex-col">
